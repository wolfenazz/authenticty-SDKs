...
namespace Authenticity {
    ...
    UpdateInfo Client::CheckForUpdate() {
        ...
        if (!IsTrue(GetJsonValue(response, "success"))) {
            m_LastError = JsonError(response, "Update check failed");
        } else {
            m_LastError.clear();
        }
        return info;
    ...
    bool Client::Register(const std::string& username, const std::string& password, const std::string& licenseKey) {
        ...
        std::string response = SendRequest("/auth/register", "POST", body.dump());
    ...
    std::vector<unsigned char> Client::DownloadFile(const std::string& fileId) {
        ...
        std::vector<unsigned char> bytes = DownloadBytes(url);
        if (bytes.empty() && m_LastError.empty()) m_LastError = "Download returned no data";
        return bytes;
    }
    ...
    void Client::Log(const std::string& data, const std::string& type) {
        ...
        if (!IsTrue(GetJsonValue(response, "success"))) {
            m_LastError = JsonError(response, "Failed to add log");
        }
    }
    ...
    std::string Client::GetHwid() const {
        return m_Hwid;
    }
    ...
    bool Client::CheckBlacklist() {
        ...
        std::string message = GetJsonValue(response, "message");
        const std::string explicitBlacklisted = GetJsonValue(response, "blacklisted");
        if (IsTrue(explicitBlacklisted) || IsBlacklistedResponse(response, message)) {
    ...
    bool Client::IsBlacklistedResponse(const std::string& response, const std::string& message) {
        ...
        const std::string explicitBlacklisted = GetJsonValue(response, "blacklisted");
        if (IsTrue(explicitBlacklisted)) return true;
